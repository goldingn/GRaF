graf.fit.laplace <-
  function (y, x, mn, l, wt, e = NULL, tol  = 10 ^ -6, itmax = 50,
    ...
    # get gradients of the objective wrt l
    
    # gradients of the kernel
    K_grads <- cov.SE.d1(x, e, l)
    
    # gradient components
    c <- backsolve(L, rW)
    R <- c %*% t(c)
    cK <- c %*% K
    C2 <- diag(K) - colSums(cK ^ 2)
    
    # first and third gradients of the log-likelihood
    # this is for the logit!!
    f_d1 <- d1(f, y)
    ef <- exp(f)
    d3 <- (ef * (ef - 1)) / (1 + ef) ^ 3
    ...
    for (i in 1:length(l)) {
      ...
      # ignores mean function!
      s1a <- 0.5 * t(a) %*% K_grads[[i]] %*% a
      s1b <- -0.5 * sum(R * K_grads[[i]])
      s1 <- (s1a + s1b)[1, 1]
      
      s3 <- b - K %*% (R %*% b)
      
      l_grads[i] <- -(s1 + t(s2) %*% s3)
    }
